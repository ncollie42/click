...
export function build(g, add){
  ...
  const base=add(meshOf(new THREE.CylinderGeometry(.66,.80,.28,8),flat(PAL.masonryDark)));base.position.y=GROUND_Y+.14;
  const pole=add(meshOf(new THREE.CylinderGeometry(.09,.12,1.85,8),flat(PAL.timberDark)));pole.position.y=GROUND_Y+1.20;
  for(const y of [.85,1.25,1.65]){
    const chevron=add(meshOf(new THREE.ConeGeometry(.34,.30,4),flat(PAL.timber)));
    chevron.position.y=GROUND_Y+y;chevron.rotation.y=Math.PI/4;
  }
  const tip=add(meshOf(new THREE.ConeGeometry(.26,.44,6),flat(PAL.coin,{emissive:PAL.arcaneGlow})));
  tip.position.y=GROUND_Y+2.28;g.userData.tip=tip;
}
