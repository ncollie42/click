...
const STEADY_HAND_RATE=1.8;  // the "steady hand" upgrade's fill multiplier
...
const BASE_ZONE=240;
// ── Placement grid ──
// One shared lattice for every placeable object. The simulation owns it; rendering consumes it.
...
const CELL=32;
const GRID_ORIGIN_X=-CELL/2,GRID_ORIGIN_Y=-CELL/2;   // world coords of cell (0,0)'s top-left corner
const GRID_COLS=W/CELL+1,GRID_ROWS=H/CELL+1;         // 49 x 33 addressable cells, edge ones half-clipped
// ── Footprint ownership ──
// Written by: the literals below and the BUILDING_TYPES / world-node definitions that reference them.
// Read by:    the grid helpers, and later canPlace() and the Three.js layer.
...
const FOOTPRINT_1x1=Object.freeze({w:1,h:1});
const FOOTPRINT_3x3=Object.freeze({w:3,h:3});
const RESOURCE_FOOTPRINT=FOOTPRINT_1x1;              // trees, rocks and diamonds each occupy one cell
// The base reserves cells exactly like a placed building - no keep-out circle, no special case in
// canPlace(). Its anchor is already a cell center, so the 3x3 is symmetric around it.
BASE.footprint=FOOTPRINT_3x3;
// ── Grid helpers (pure) ──
// No state reads, no mutation. Cell coords (cx,cy) are integers; world coords are sim pixels.
...
const worldToCell=(x,y)=>({cx:worldToCellX(x),cy:worldToCellY(y)});
const cellToWorldX=cx=>GRID_ORIGIN_X+cx*CELL+CELL/2;
const cellToWorldY=cy=>GRID_ORIGIN_Y+cy*CELL+CELL/2;
const cellToWorld=(cx,cy)=>({x:cellToWorldX(cx),y:cellToWorldY(cy)});
...
function snapToCellCenter(x,y){const c=worldToCell(x,y);return cellToWorld(c.cx,c.cy);}
// Footprints are odd, so half-extent is a whole number of cells on each side of the anchor.
const footprintHalf=size=>(size-1)/2;
function buildingFootprint(type){return BUILDING_TYPES[type]?.footprint||FOOTPRINT_1x1;}
function footprintCellBounds(cx,cy,footprint=FOOTPRINT_1x1){
...
function footprintCells(cx,cy,footprint=FOOTPRINT_1x1){
...
// World-space rectangle covered by a footprint; the later rendering/placement consumer of {w,h}.
function footprintWorldRect(cx,cy,footprint=FOOTPRINT_1x1){
...
function footprintInWorldBounds(cx,cy,footprint=FOOTPRINT_1x1){
  const r=footprintWorldRect(cx,cy,footprint);
  return r.x>=0&&r.y>=0&&r.x+r.w<=W&&r.y+r.h<=H;
}
const HOUSE_SLOTS=2,HOUSE_COST={wood:8,stone:6},HOUSE_COST_ESCALATION={wood:4,stone:3},WORKER_SPAWN_TIME=12;
...
const BUILD_MARGIN=45;        // buildable world = the map inset by this much on every side
