...
export const CARDS=[
  ...
  {id:"towerRange",   category:"buff", rarity:"rare",   text:"all towers +50 attack radius",
  ...
  {id:"fireball",     category:"consumable", rarity:"rare",   text:"slam down 3 fireballs for 5 damage each",
   ...
   notes:"held in hand; 3 casts using the blast placement ghost; each ball falls for FIREBALL.fallTime, then deals 5 area damage and leaves no building"},
  ...
  {id:"woodBundle",   category:"consumable", rarity:"common", text:"+20 wood, delivered now",
   features:["resources"], ref:"concept:bundle", model:null, implemented:true, inPool:false, notes:"disabled: stored resources are primarily feed XP, so an instant bundle is not a meaningful reward"},
  ...
  {id:"bpWardTotem",category:"build",rarity:"rare", text:"place a ward totem — nearby towers gain 10 hp", tags:["aoe"], features:["physical space"], charges:1, ref:"building:wardTotem", model:null, implemented:true, inPool:true, notes:"persistent 1x1 support building; complete towers within 128px gain +10 max hp granted as real health; leaving the aura removes the pool but never drops a tower below 1 hp; copies do not stack"},
  {id:"bpHasteTotem",category:"build",rarity:"rare", text:"place a haste totem — nearby towers attack 25% faster", tags:["aoe"], features:["physical space"], charges:1, ref:"building:hasteTotem", model:null, implemented:true, inPool:true, notes:"persistent 1x1 support building; complete towers within 128px reload at 0.8x cooldown; copies do not stack"},
  {id:"bpLumber",    category:"build", rarity:"common", text:"place a lumber camp",features:["resources","physical space"], charges:1, ref:"building:lumber",    model:null, implemented:true, inPool:true, notes:"3x3 work building — grows NOTHING; two job slots the scheduler auto-fills (or drag a worker on) work wild trees inside its 155px service radius at 1/s vs the free worker’s 1 per 2s ceiling (STAFF_GATHER), so place it beside a real forest"},
  {id:"bpQuarry",    category:"build", rarity:"common", text:"place a quarry",    features:["resources","physical space"], charges:1, ref:"building:quarry",    model:null, implemented:true, inPool:true, notes:"3x3 work building — grows NOTHING; two job slots the scheduler auto-fills (or drag a worker on) work wild rocks inside its 155px service radius at 1/s vs the free worker’s 1 per 2s ceiling (STAFF_GATHER), so place it beside a real rockfield"},
