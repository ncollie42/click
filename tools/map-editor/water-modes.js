...
// comparison. Each build tears down exactly what it created; the render target
// lives only while depth water is active. Browser-only.
...
export function createWaterModes({renderer, surfaceY, tileOf, floorYOf}){
  ...
  const targets = {depth: null};          // render target, ditto
  ...
  function clear(){
    ...
    owned.length = 0;
    if(targets.depth){ targets.depth.depthTexture?.dispose(); targets.depth.dispose(); targets.depth = null; }
  }
  ...
  function buildDepthFoam(){
    ...
    // A lake floor at shore-wall depth: without it, view rays escape past the
    // wall bottoms into the void, and the shallow→deep gradient snaps to full
    // deep in a hard line. The floor keeps thickness finite and continuous, so
    // fade + shore depth control the entire gradient run.
    const tile = tileOf(doc);
    const floorGeometry = own(new THREE.PlaneGeometry((doc.width + 6) * tile, (doc.height + 6) * tile));
    const floor = new THREE.Mesh(floorGeometry, own(new THREE.MeshLambertMaterial({color: FLOOR_COLOR})));
    ...
    perFrame = camera => {
      ...
      if(!targets.depth || targets.depth.width !== size.x || targets.depth.height !== size.y){
        targets.depth?.depthTexture?.dispose();
        targets.depth?.dispose();
        targets.depth = new THREE.WebGLRenderTarget(size.x, size.y, {depthTexture: new THREE.DepthTexture(size.x, size.y)});
      }
      material.uniforms.uDepth.value = targets.depth.depthTexture;
      ...
      // Depth pre-pass: everything but the water, cheapest possible materials.
      built.visible = false;
